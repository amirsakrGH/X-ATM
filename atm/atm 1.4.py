...
class SampleApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.shared_data = {'Balance':tk.IntVar(),'Cash':tk.IntVar()}
...
# ################################################################
        ...
        withdraw_issue_label = tk.Label(self,
                                            text='',
                                            font=('orbitron', 12),
                                            foreground='white',
                                            background='#33334d',
        ...
        withdraw_issue_label.pack(fill='both', expand=True)
        ...
        def other_amount(_):
            ...
            global current_balance
            global cash_notes
            if (withdraw(int(cash.get())) <= 50) and current_balance >= int(cash.get()):
                # global current_balance
                current_balance -= int(cash.get())
                result_val += int(cash.get())
                cash_notes = result_val
                controller.shared_data['Cash'].set(cash_notes)
                print(cash_notes)
                controller.shared_data['Balance'].set(current_balance)
                cash.set('')
                controller.show_frame('ApprovedPage')
            elif (withdraw(int(cash.get())) > 50) and (current_balance >= int(cash.get())):
                withdraw_issue_label['text'] = 'Cannot issue more than 50 banknotes.'
                cash.set('')
            elif (withdraw(int(cash.get())) <= 50) and current_balance < int(cash.get()):
                withdraw_issue_label['text'] = 'Sorry, You have insufficient funds available.'
                cash.set('')
            elif (withdraw(int(cash.get())) > 50) and current_balance < int(cash.get()):
                withdraw_issue_label['text'] = 'Sorry, You have insufficient funds available.\nCannot issue more than 50 banknotes.'
                cash.set('')
        other_amount_entry.bind('<Return>', other_amount)
        ...
        def withdraw(amount):
            ...
            while amount >= 10:
                if amount >= 200 and (total_notes == 0 or two_hundred_notes / total_notes < 0.6):
                    ...
                    ten_notes += 1
                    total_notes += 1
            return f"200s: {two_hundred_notes}, 100s: {hundred_notes}, 50s: {fifty_notes}, 20s: {twenty_notes}, 10s: {ten_notes}"

# ----------------------------------------------------------------
        global cash_notes
        cash_value = controller.shared_data['Cash'].get()
        result_text = withdraw(cash_value)
